...
class AgentRuntime:
    ...
    async def run(self, messages: list[dict[str, Any]]) -> str:
        ...
        for _ in range(MAX_TOOL_ROUNDS):
            ...
            for _, name, arguments, call_id in normalized_calls:
                ...
                # ToolRegistry exposes execute(); keep the agent dependent on
                # the public registry API rather than a nonexistent method.
    ...
    async def _request(self, messages: list[dict[str, Any]]) -> dict[str, Any]:
        response = await self.llm.chat(messages, tools=self.tools.definitions())
